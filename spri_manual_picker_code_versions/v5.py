...
Changes:
- poly draw 
    - right-clicking shows the specified area with the edge filter    
...
class Window:
    ...
    def __init__(self, windowName: str, image: cv2.Mat = None, drawable=False,drawColourSurface: tuple = (0, 0, 255), drawColourBed: tuple = (0, 255, 0),DEBUG: bool = False):
        """
...
class PolygonDrawer():
    ...
    def on_mouse(self, event, x, y, buttons, user_param):
        ...
        if event == cv2.EVENT_MOUSEMOVE: # on MOUSE:MOVE, save the new position 
            ...
            self.window.displayedImage = Image.mask(Image.edgeC(65,self.window.sourceImage), cv2.cvtColor(self.stencil, cv2.COLOR_BGR2GRAY) )
...
if __name__ == "__main__":
    ...
    # creating windows
    winRef = Window("Reference", drawable=True, image=image.img, DEBUG=DEBUG)
