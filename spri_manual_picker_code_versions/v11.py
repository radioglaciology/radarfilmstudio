...
class Window:
    ...
    def __init__(self, windowName: str, image: Image = None, batchsize = 5, drawable=False, detail=10, drawColourSurface= (255, 0, 0), drawColourBed = (0, 255, 0), DEBUG = False):
        """
        ...
        if drawable: # creates the necessary drawing components 
            ...
            self.lineDrawerCBD = cbdSelector(self, (150,150, 100), DEBUG= self.DEBUG)
            self.drawDone = False
            self.detail = detail
            self.cbd_boarders = []
    ...
    def filterImage(self, image:cv2.Mat):
        ...
        if not self.drawDone:
            ...
            for line in self.lineDrawerCBD.points:
                cv2.line(image, (line[0],0) , (line[0], image.shape[1]), self.lineDrawerCBD.colour, 2)
    ...
    def surfaceBedSwitch (self, switch):
        ...
        elif switch == 1:  # enables drawing and undos for the bed
        ...
        elif switch == 2:  # enables drawing and undos for the bed

            if self.DEBUG: print("Drawing bed")

            self.polyDrawerSurface.done = True
            self.polyDrawerBed.done = True
            self.lineDrawerCBD.done = False

            cv2.setMouseCallback(self.windowName, self.lineDrawerCBD.on_mouse) # makes cv2 listen to mouse inputs
    ...
    def drawLines (self):
        ...
        if len(self.polyDrawerSurface.points) >= 3: # make sure there are enough points for create an avg line selection
            ...
            self.surfacePoints= self.polyDrawerSurface.createAvgLine(detail=self.detail, area="Surface")
        ...
        if len(self.polyDrawerBed.points) >= 3: # make sure there are enough points for create an avg line selection
            ...
            self.bedPoints= self.polyDrawerBed.createAvgLine(detail=self.detail, area="Bed")
        
        self.writeToCSV(self.surfacePoints, self.bedPoints)
    ...
    def undoOnChange(val, targetWindow):
        ...
        elif targetWindow.active == 1:
            if targetWindow.DEBUG: print("Undoing for bed")
            targetWindow.polyDrawerBed.points.pop()
        
        elif targetWindow.active == 2:
            if targetWindow.DEBUG: print("Undoing for cbd selector")
            targetWindow.lineDrawerCBD.points.pop()
    ...
    def writeToCSV(self, data_surface, data_bed):
        CBD_TICK_PADDING = 35
        """
        ...
        points_surface = [] # list that stores the pixel depths of each averaged point
        points_bed = [] # list that stores the pixel depths of each averaged point
        ...
        for point in data_surface:
            points_surface.append((point[0],point[1]-CBD_TICK_PADDING, img[point[1]-1, point[0]-1]))
        
        for point in data_bed:
            points_bed.append((point[0],point[1]-CBD_TICK_PADDING, img[point[1]-1, point[0]-1]))
        # prep for writing into csv

        imgName = (self.image.imgPath.split("/")[-1]).split(".")[0] # extract the file name from the image path string 
        frame_range = imgName.split("_")[1:3]
        print(frame_range)
        ...
        x_surface.append("x_surface")
        y_surface.append("y_surface")
        depth_surface.append("pixel_depth_surface") 

        x_bed.append("x_bed")
        y_bed.append("y_bed")
        depths_bed.append("pixel_depths_bed")
        ...
        for i in range(len(points_surface)):
            ...
            delta_y.append(points_surface[i][1]-points_bed[i][1])


        data = [x_surface,y_surface,depth_surface,x_bed,y_bed,depths_bed,delta_y]
...
class PolygonDrawer():
    ...
    def __init__(self, window: Window, colour: tuple, batchsize = 5, DEBUG = False):
        """
    ...
    def createAvgLine(self, detail:int, area:str):
        ...
        # for n in range(detail): # for each subsection
            ...
            # boarder is set to be +- 20px around each cbd line indicator
        for boarder in self.window.cbd_boarders: 
            ...
            # 
            MIN_NUMBER_OF_POINTS_TO_DETECT = 10
            if pointsInRange > MIN_NUMBER_OF_POINTS_TO_DETECT:
...
class cbdSelector():
    ...
    def on_mouse(self, event, x, y, buttons, user_param):
        ...
        if event == cv2.EVENT_MOUSEMOVE: # on MOUSE:MOVE, save the new position 
            ...
            self.points.append((x, y)) # adds the mouse location to the points list
            self.window.cbd_boarders.append((x,y))
            self.window.cbd_boarders.sort()
            self.window.detail += 1
            # cv2.polylines(self.window.displayedImage, np.array([self.points]), False, self.colour, 3) # create the temp-poly lines
            cv2.line(self.window.displayedImage, (self.position[0], 0), (self.position[0], self.window.displayedImage.shape[1]), self.colour, 20)
            self.window.refresh(self.window.displayedImage)# display the new temp-poly
...
class App:
    ...
    def process(self, tiff):
        ...
        DETAIL = 0 #initial detail value


        imgPath = self.path + "/" + tiff #compose the image path from the folder and the items within

        # base image:=========================
        # image = Image(imgPath, cropVals=(0,94), DEBUG=DEBUG) #DEV/DEBUG SETUP

        # image = Image(imgPath, cropVals=(0,824), DEBUG=DEBUG) #DEV/DEBUG SETUP
        image = Image(imgPath, cropVals=(425, 1780), DEBUG=DEBUG)
        ...
        winRef.setTrackbar("Threshold", 10) # sets the trackbar's initial value 
        ...
        winRef.addTrackbar("S/B/CBD Select", (0, 2), winRef.surfaceBedSwitch)
    ...
    def launch(self, saveDirectory = ""):
        ...
        while anchorCounter < len(self.dir_list):
            ...
            for count in range(self.batchSize):
                # if cv2.getTrackbarPos("Rewind", "Reference") == 1:
...
if __name__ == "__main__":
    # source = "C:\\Users\\LENOVO\\Box\\Summer2022\\Flight_Images_Fall2022\\Flight103\\otherleg"
    # destination = "C:/Users/LENOVO/Box/Summer 2022/Flight Images_Fall2022/103Processed"

    # DEV/DEBUG SETUP
    # source = "C:/Users/LENOVO/Desktop/testImages Sequence"
    source = "C:\\Users\\LENOVO\\Desktop\\TIFFS"
    destination = "C:/Users/LENOVO/Desktop/saveHere/"

    # ACTUAL SETUP
    app = App(source, batchSize=1)
