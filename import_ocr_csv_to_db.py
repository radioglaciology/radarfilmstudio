    ...
    for idx, r in df.iterrows():
        ...
        if q.count() == 0: # This path doesn't exist in the db yet
            ...
            fseg = FilmSegment(path=r['relpath'], reel=r['reel'], first_frame=r['first_frame'], last_frame=r['last_frame'],
                            ...
                            raw_date=r['first_date'], raw_time=r['first_time'], raw_mode=r['first_mode'],
                            is_junk=False, is_verified=False, needs_review=False, dataset=dataset,
                            scope_type=r['type'], instrument_type=FilmSegment.UNKNOWN, notes="",
                            updated_by=updated_by_name, last_changed=datetime.now())
            db.session.add(fseg)
        else:
            seg = q.first()
            ...
            if seg.last_frame is None and (not np.isnan(r['last_frame'])):
                ...
                updated = True
            if seg.first_cbd is None and (not np.isnan(r['first_cbd'])):
                seg.first_cbd = r['first_cbd']
                updated = True
            if seg.last_cbd is None and (not np.isnan(r['last_cbd'])):
                seg.last_cbd = r['last_cbd']
                updated = True
